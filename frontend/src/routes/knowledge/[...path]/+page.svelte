      ...
      {#snippet primary()}
        <div class="flex min-h-0 min-w-0 flex-1 flex-col">
          <!-- The primary's own header band. `PaneHost` renders one around
               ...
               side pane open this band runs to the window's right edge. The
               `, 0px` fallback is required, not defensive: a `calc()` against
               an undefined custom property is invalid at computed-value time
               and the browser drops the whole `padding-right`, base px-3
               included. -->
          <div
          ...
          >
            ...
            {#if mountKey}
              <SessionPickerPopover
                ...
                onNewSession={() => wiring.openBeside({ kind: 'chat-new', mountKey, from: null })}
