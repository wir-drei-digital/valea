        ...
        <div class="border-paper-border bg-paper-card flex flex-col gap-3 rounded-lg border p-4">
          ...
          {:else if inspection.adoptable}
            <!-- Task 13 (Spec D §D4): the adopt-a-folder consent step —
                 ...
                 could adopt. The ONE file this STEP writes is the identity
                 file the copy names. The briefing sentence is the
                 tasks+schedules spec's §Discovery requirement (see the
                 identical copy in `OpenWorkspaceFlow.svelte` for why the
                 adopt flow is where it belongs). -->
            <div class="flex flex-col gap-2.5">
              <p class="text-ink-body text-[13px]">
                This folder isn't a Valea ICM yet. Add a small identity file (icm.yaml) so Valea can
                recognize this folder — that's the only file this step writes. Once it's active,
                Valea also keeps a contract file for agents at .valea/briefing.md, describing the
                tasks.json and schedules.json files they may write.
              </p>
