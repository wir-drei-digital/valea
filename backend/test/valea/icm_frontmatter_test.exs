defmodule Valea.ICMFrontmatterTest do
  ...
  setup do
    ...
    File.write!(Path.join(ws, "icm/Workflows/Contract.md"), @page_with_fm)
    File.write!(Path.join(ws, "icm/Workflows/Broken.md"), @page_broken_yaml)
  ...
  describe "page/1 + save_page/3 with frontmatter" do
    test "page returns parsed frontmatter, whole-file content, body-only prosemirror" do
      {:ok, page} = ICM.page("Workflows/Contract.md")
    ...
    test "save without edits reattaches frontmatter byte-identically (round trip)" do
      {:ok, page} = ICM.page("Workflows/Contract.md")
      {:ok, _} = ICM.save_page("Workflows/Contract.md", page.prosemirror, page.hash)
      ...
      {:ok, page2} = ICM.page("Workflows/Contract.md")
      assert String.starts_with?(page2.content, "---\nenabled: true\nrisk_level: medium\n---\n")
      {:ok, _} = ICM.save_page("Workflows/Contract.md", page2.prosemirror, page2.hash)
      {:ok, page3} = ICM.page("Workflows/Contract.md")
      assert page3.content == page2.content
    end

    test "malformed yaml -> frontmatter nil, page still readable" do
      {:ok, page} = ICM.page("Workflows/Broken.md")
