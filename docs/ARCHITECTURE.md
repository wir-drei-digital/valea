...
adjusts it in the Settings dialog's Agent section (sidebar gear:
`SettingsModal.svelte` → `AgentSection.svelte` — harness command + the same
doctor checks),
...
high. UI: a Skills section in the Settings dialog's Agent section
