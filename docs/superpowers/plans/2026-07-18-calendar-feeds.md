...
- ACTIVATION ORDER (the amended spec §Sync engine bullet): (1) derive UNCONDITIONALLY — parse the committed `feed.ics` (if present) and rebuild views + index from it, no marker consultation, no credential needed; this repairs out-of-band damage even when both markers still match. (2) Credential install: read the keychain/env-supplied URL if one is available (RAM closure). (3) ONLY with a URL in hand, `Source.verify_or_claim` (absent → claim; mismatch → inert `identity_mismatch`); with NO URL, `.source` stays UNTOUCHED and the engine idles with `url_present: false` until resupply via `set_url`. (4) Poll timer starts only when a URL is present and identity verified. Restart tests: missing credential (idle, no claim written, derive still ran), matching credential (verified → polling), mismatching credential (inert `identity_mismatch`, mirror intact).
