...
defmodule Valea.Calendar.Settings do
  defstruct sources: %{}, feed_token_hash: nil
...
- EVERY pass — including 304 `:unchanged` — and activation: compute `rev` from the CURRENT `feed.ics` bytes + today's window; if `Views.current_rev != rev` OR `Store.derived_rev(slug) != rev` → re-derive both stores from `feed.ics`. This is what heals crashes between swaps, failed derives behind 304s, host-zone changes, day rollover, and window-config changes.
- `prev_event_count` lives in engine state, seeded at activation by the unconditional re-derive's parse of `feed.ics` (absent file → 0).
...
- [ ] **Step 1: tests first** — the mail_rpc_test shape for every action: happy path, generation guard, slug grammar rejects incl. `valea` on setup/set-url/purge, purge refusals (still-configured; unconfigured-but-never-existed), typed confirms, `list_calendar_events` range tests (timed event straddling range start INCLUDED; UTC-date-vs-local-date boundary event lands on the correct local day for a negative-offset zone; all-day exclusive-end overlap; mixed ordering all-day-first), invalid `zone` rejected, doctor check gating + no-URL-in-output, cockpit line presence + leniency, channel push shapes.
...
// setup actions (setupCalendarSource, keychainSet(workspaceId, `${slug}:ics`, url)
// then setCalendarSourceUrl, removeCalendarSource, purgeCalendarSourceFiles,
...
- [ ] **Step 1: vitest first** for the adapter (UTC→local split across a day boundary in a non-UTC zone, multi-day timed split, all-day exclusive split, kind mapping, DST-day segment lengths) and the store (merge/refresh on pushes, editor inclusive↔exclusive round-trip, feed block state).
