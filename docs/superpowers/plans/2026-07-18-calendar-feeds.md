...
- A pass: conditional GET → parse → `Ics.acceptable?(feed, Store.occurrence_count(slug) > 0)` → atomic `feed.ics` swap (tmp + fsync + rename) → derive → broadcast. Derive = `Views.rebuild!` (views swap FIRST) then `Store.replace_source!` (SQLite transaction SECOND, writes `derived_rev = rev`).
- ACTIVATION ORDER (the amended spec §Sync engine bullet): (1) an unconditional repair ATTEMPT — parse the committed `feed.ics` (if present), no marker consultation, no credential needed — that is an unconditional attempt, NOT an unconditional commit: the rebuild of views + index runs ONLY after parse succeeds AND `Ics.acceptable?(feed, Store.occurrence_count(slug) > 0)` passes (the pre-derive count is the "previous had events" evidence); a truncated/non-ICS/mostly-malformed on-disk snapshot leaves BOTH derived stores untouched and marks the source degraded with the parse reason — the spec's failure posture (parse failures never destroy the previous mirror) applies to activation exactly as to passes. This repairs out-of-band damage to DERIVED files even when both markers still match, without letting a damaged snapshot erase a healthy mirror. (2) Credential install: read the keychain/env-supplied URL if one is available (RAM closure). (3) ONLY with a URL in hand, `Source.verify_or_claim` (absent → claim; mismatch → inert `identity_mismatch`); with NO URL, `.source` stays UNTOUCHED and the engine idles with `url_present: false` until resupply via `set_url`. (4) Poll timer starts only when a URL is present and identity verified. Restart tests: missing credential (idle, no claim written, derive still ran), matching credential (verified → polling), mismatching credential (inert `identity_mismatch`, mirror intact), and SNAPSHOT-DAMAGE activation cases with AND without a credential: `feed.ics` overwritten with non-ICS bytes → both derived stores intact + degraded; overwritten with a partial-damage feed (1 valid + several malformed VEVENTs) over a populated mirror → intact + degraded; a legitimately-empty valid snapshot over an empty mirror → clean rebuild; and DAMAGED-SNAPSHOT-THEN-304 (restart with credential onto a damaged `feed.ics` → activation preserves both stores; the next poll answers 304 → marker mismatch triggers the repair attempt → the SAME guard rejects again → both stores STILL unchanged, degraded persists).
- EVERY subsequent pass — including 304 `:unchanged` — runs the marker check: compute `rev` from the CURRENT `feed.ics` bytes + today's window; if `Views.current_rev != rev` OR `Store.derived_rev(slug) != rev` → attempt re-derive of both stores from `feed.ics`.
- ONE GUARDED DERIVE FUNCTION serves all three snapshot→derived-store paths (activation, post-fetch derive, 304/marker repair): parse → `Ics.acceptable?(feed, Store.occurrence_count(slug) > 0)` → rebuild both stores; on parse or guard failure, BOTH stores stay untouched and the source goes degraded with the reason. Previous-event evidence is ALWAYS the live `Store.occurrence_count/1` read at guard time — never an engine-state counter (a rejected activation must not fake "no previous events" into a later pass). `unsupported_series` in status comes from the LAST SUCCESSFUL derive (0 until one happens). The post-fetch path additionally runs this admission BEFORE the `feed.ics` swap (a rejected response never becomes the committed snapshot). This heals crashes between swaps, failed derives behind 304s, host-zone changes, day rollover, and window-config changes — without ever letting a damaged snapshot erase a healthy mirror.
...
- `CalendarSetupPanel`: source list with per-source status/doctor/typed-confirm purge — the status line renders the spec-mandated "N series unsupported" whenever `unsupported_series > 0` (the field rides Engine.status through `calendar_status`) — add-source form (slug, name, URL — the store's pinned three-step sequence VERBATIM: `setup_calendar_source` → `set_calendar_source_url` → `keychainSet` on acceptance; "set-url first" means first RELATIVE TO KEYCHAIN PERSISTENCE only, never before setup, which creates the engine set-url talks to), served-feed block (enable, URL + copy, rotate, and the honest reachability copy: local calendar apps can subscribe; server-side fetchers — iCloud/Google/Outlook.com — cannot reach loopback, so no phone propagation in this phase). Store test: a source status with `unsupported_series: 2` renders the notice.
